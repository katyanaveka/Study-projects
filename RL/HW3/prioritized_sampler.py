...
class SumTree():
    ...
    def __getitem__(self, indices):
        return self.tree[np.array(indices) + self.capacity - 1]
...
class PrioritizedSampler():
    def __init__(self, size, clip_priorities=1.0, rp_alpha=0.4):
        ...
        self._size = size ###
        self.beta = 0 # will be increased in every sample_indices call (from uniform (0) to priority (1))
    ...
    def sample_indices(self, batch_size):
        ...
        """
        '''
        self.beta = min(1, self.beta + 1e-4)
        
        # sample batch_size indices
        # indices = np.random.choice(self._size, batch_size, p=self.priorities) ###
        indices = []

        delta = self.priorities.total_p / batch_size
        for i in range(batch_size):
            v = np.random.uniform(delta * i, delta * (i + 1))
            idx = self.priorities.get_leaf(v)
            indices.append(idx)
        
        priorities = self.priorities[indices]
        
        # you can change this to implement bias correction
        weights = 1 / priorities ** self.beta #np.ones(batch_size)
